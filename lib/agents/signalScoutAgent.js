...
function buildResearchQueue(inputs) {
  ...
  const socialTasks = inputs.raw.xSocialSignals
    .filter(shouldCreateSocialResearchTask)
    .map(buildSocialTask);
...
function buildSocialTask(signal) {
  ...
  return {
    ...
    ticker,
    displayTicker: signal.ticker || signal.cashtag || signal.relatedTickers?.[0] || signal.tickers?.[0] || ticker,
    sourceType: "x_social",
    priority: signal.recommendedNextStep === "send_to_research" ? "MEDIUM" : "WATCH",
    theme: signal.relatedThemes?.[0] || signal.theme || signal.category || "Social/media signal",
    catalyst: signal.claim || signal.catalyst || signal.signal || "Social mention requires verification",
    whyNow: signal.claim || signal.signal || "X/social signal observed",
    requiredVerification: signal.requiredVerification || [
    ...
    xSignalCategory: signal.category,
    xRecommendedNextStep: signal.recommendedNextStep,
...
function shouldCreateSocialResearchTask(signal) {
  const ticker = signal.relatedTickers?.[0] || signal.tickers?.[0] || signal.ticker || signal.cashtag;
  if (!ticker) return false;
  return ["send_to_research", "send_to_skeptic", "watch"].includes(signal.recommendedNextStep);
}
